...
from electric_weight import DistributedFanWeight, ElectricEquipWeight
...
class InitEngineWeightParamsCore(InitEngineParamsCore):

    def __init__(self, engine_name, propulsion_type, engine_data_path):
        ...
        stage_number_data = self.engine_json_file['stage_number']
        self.fan_stage_number = stage_number_data['Fan']
        self.lpc_stage_number = stage_number_data['LPC']
        self.hpc_stage_number = stage_number_data['HPC']
        self.hpt_stage_number = stage_number_data['HPT']
        self.lpt_stage_number = stage_number_data['LPT']

        # Aspect ratio (default)
        self.aspect_ratio_rotor = 2.2
        self.aspect_ratio_stator = 2.8

        # load stage coefficients from engine database file
        # {"stage_coef": {"LPC": 0.138, "HPC": 0.3635, "HPT": 1.29, "LPT":0.484}}
        # Stage load coefficients(need tuning for the same public value of targeting engine)
        stage_coef_data = self.engine_json_file['stage_coef']
        self.lpc_stage_coef = stage_coef_data['LPC']
        self.hpc_stage_coef = stage_coef_data['HPC']
        self.hpt_stage_coef = stage_coef_data['HPT']
        self.lpt_stage_coef = stage_coef_data['LPT']

        # Tip velocity
        self.u_tip_fan = self.fan_tip_velocity[0]
        self.u_tip_lpc = self.lpc_tip_velocity
        self.u_tip_hpc = self.hpc_tip_velocity
        self.u_tip_hpt = self.hpt_tip_velocity
        self.u_tip_lpt = self.lpt_tip_velocity

        # Duct length
        self.duct_length = 1.5  # [m]

        # Design thinking
        # inside diameter constant:0 outside diameter constant:1 mean diameter constant:2
        # shape: {component index: upper number,...}
...
# References
# ToDo: insert Nasa thesis url

# Integration Engine weight class

class EngineWeight(object):

    def __init__(self, aircraft_name, engine_name, aircraft_type, propulsion_type, engine_data_path, calc_design_point_class, calc_off_design_point_class):
        ...
        self.dfw = DistributedFanWeight(design_point_results, off_design_point_results, [self.fscl])
        ...
        self.eew = ElectricEquipWeight(design_point_results, off_design_point_results,
        ...
        self.distributed_fan_width_length = self.dfw.distributed_fan_width_length
    ...
    def run_engine(self):
        ...
        if self.propulsion_type in ['turbojet', 'turboshaft', 'turbofan']:
            ...
            self.dfw.run_distributed_fan_weight()

            # insert weight results
            self.weight_results[110] = self.dfw.distributed_fan_weight
            self.weight_results[111] = self.dfw.distributed_fan_duct_weight

            # Operate Electric equipment calculation
            self.eew.run_electric_equip()

            # calculate structure and controller weight
            self.calc_structure_and_controller_weight()

            # Insert weight results
            self.weight_results[99] = self.eew.electric_equip_weight
        ...
        print('distributed fan diameter:', self.dfw.fan_in_diameter[0])
    ...
    def calc_diameter(self):
        ...
        # hpt
        # Out
        hpt_out_diams = turb_diameter_no_constraints(41, self.iewpc.hpt_tip_velocity, self.hp_shaft_rpm,
                                                     self.outlet_areas)
    ...
    def calc_core_weight(self):
        ...
        hpt_length, hpt_weight = turbine_indexes((40, 41), hpt_stage_number, diameters, self.iewpc.hpt_tip_velocity,
                                                 hpt_an_coefs)
        ...
        lpt_length, lpt_weight = turbine_indexes((45, 46), lpt_stage_number, diameters, self.iewpc.lpt_tip_velocity,
                                                 lpt_an_coefs)
...
def test():
    ...
    dv = DesignVariable(
        propulsion_type, aircraft_type
    )

    thermal_design_variables = dv.set_design_variable(dv_list)

    """
    # build class object for calculating design point performances
    cdp = CalcDesignPoint(
        aircraft_name, engine_name, aircraft_type, propulsion_type, thermal_design_variables
    )
    ...
    # calculate off design point performance
    cdop.run_off(rev_args)

    # calculate off design index
