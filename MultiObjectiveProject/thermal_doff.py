...
def get_component_classes(propulsion_type):
    """
...
class CalcDesignOffPoint(CalcDesignPoint):
    """
    Note:
    ...
    def __init__(self, aircraft_name, engine_name, aircraft_type, propulsion_type, thermal_design_variables,
                 off_param_args, design_point_params, data_base_args, calc_design_point_class=None):
        ...
        if calc_design_point_class is None:
            # set initial mission class
            init_mission_class = InitMission(aircraft_name, engine_name, self.aircraft_data_path, self.engine_data_path)
            # succeed design point
            super().__init__(aircraft_name, engine_name, aircraft_type, propulsion_type, thermal_design_variables,
                             data_base_args)

            ######### Class CalcDesignPoint operation #################
            # calc design point
            self.run_dp()

            # confirmation objective function values
            self.objective_func_dp()

            # set the design off point params
            self.define_design_off_point(off_param_args)
            ##########################################################
        # if you want to calculate design point and off design point individually,
        # you have to get calc design point class which already finish performance's computtions
        # into this class's arguments
    ...
    def build_off_design_component_classes(self):
        ...
        self.build_hp_classes = []
        self.build_lp_former_classes = []
        self.build_lp_latter_classes = []
        self.build_electric_classes = []

        if self.hp_shaft_classes != None:
            # Build Hp class

            for idx, hp_class in enumerate(self.hp_shaft_classes):
                # HPC class switch
                if idx == 0:

                    c_class = hp_class(self.coff, self.doff, self.qref, self.propulsion_type, False)

                else:

                    c_class = hp_class(self.coff, self.doff, self.qref, self.propulsion_type)

                self.build_hp_classes.append(c_class)

        if self.lp_shaft_classes != None:
            # Build off design components class objects
            self.lp_former_classes = self.lp_shaft_classes[0]
            self.lp_latter_classes = self.lp_shaft_classes[1]

            # Build lp former class

            for lp_f_class in self.lp_former_classes:
                c_class = lp_f_class(self.coff, self.doff, self.qref, self.propulsion_type)

                self.build_lp_former_classes.append(c_class)

            # Build lp latter class

            for lp_l_class in self.lp_latter_classes:
                c_class = lp_l_class(self.coff, self.doff, self.qref, self.propulsion_type)

                self.build_lp_latter_classes.append(c_class)

        if self.elec_shaft_classes != None:

            # Consider fix
            # Build electric class

            for e_class in self.elec_shaft_classes:
                c_class = e_class(self.coff, self.doff, self.coff_e, self.qref, self.qref_e, self.propulsion_type)

                self.build_electric_classes.append(c_class)
    ...
    def run_off(self, rev_args):
        ...
        if self.propulsion_type in ['turbojet', 'turboshaft', 'turbofan']:
            qoff = self.run_core_off(qoff_e, rev_lp)

        # Jet Engine + Electric
        if self.propulsion_type in ['TeDP', 'PartialElectric']:
            qoff, qoff_e = self.run_electric_off(rev_args)

        # Jet Engine + Battery
        if self.propulsion_type in ['battery', 'hybridturbojet', 'hybridturbofan']:
            qoff, qoff_e = self.run_battery_off()
    ...
    def run_core_off(self, qoff_e, rev_lp):
        ...
        angle_lp = self.init_angle_lp
        angle_lp_step = self.init_angle_lp_step

        # residual lp shaft
        # Convergence by applying two split method
        reslp = 0.0
        reslpold = 0.0
        A90_target = self.qref[2, 90]  # Target Core Engine Cross Area
        iterlp = 0  # calculation counts of the part of lp shaft
        lp_ok = True  # Convergence Flag(Boolean)
        ...
        while lp_ok:
            ...
            # prmwl == rev_lp_rate
            rev_lp_rate = self.rev_lp * (np.sqrt(2.0) * np.cos(angle_lp)) ** (2.0 / beta)

            iterlp += 1

            # run_former()
            qoff, rev_rate, rev_lp = self.run_lp_former(qoff, qoff_e, rev_lp_rate, self.rev_lp)
            ...
            angle_hp = self.init_angle_hp
            angle_hp_step = self.init_angle_hp_step
            ...
            while hp_ok:
                ...
                qoff, rev_rate, rev_hp = self.run_hp(qoff, qoff_e, rev_rate25, rev_hp)

                # prepare for checking energy matching
                ytm40 = self.coff[5, 40]
                ytdah = self.coff[4, 25]
                ytd25 = self.doff[51]
            ...
            qoff = self.run_lp_latter(qoff, qoff_e)
        ...
        # print('rev_lp:',self.rev_lp,'angle_lp:',angle_lp)
        # rev_lp_rate = self.rev_lp*(np.sqrt(2.0)*np.cos(angle_lp))**(2.0/beta)
    ...
    def run_lp_former(self, qoff, qoff_e, rev_rate, rev_lp):
        ...
        for lp_f_class in self.build_lp_former_classes:
    ...
    def run_lp_latter(self, qoff, qoff_e):
        ...
        for lp_l_class in self.build_lp_latter_classes:
            qoff = lp_l_class(qoff, qoff_e)
    ...
    def run_hp(self, qoff, qoff_e, rev_rate, rev_hp):
        ...
        # Now convergence hp shaft

        for hp_class in self.build_hp_classes:
    ...
    def run_electric_off(self, rev_args):
        ...
        div_alpha = self.doff[33]
        div_alpha_step = self.div_alpha_step
        ...
        while True:
            ...
            qoff_e = self.run_distributed_fan(rev_fan)

            qoff = self.run_core_off(qoff_e, rev_lp)

            # Residual fan area
            A19 = qoff_e[2, 19]
            A19_target = self.qref_e[2, 19]
            ...
            # Normal convergence
            if abs(res_fanarea) < self.eps_fanarea:
                break
    ...
    def objective_func_doff(self):
        ...
        if self.propulsion_type in ['TeDP', 'PartialElectric']:
            ...
            self.GenePower = max(self.qoff_e[6, 10], self.qref_e[6, 10]) * self.fscl / ytaele / 1.0e+3
        ...
        # Thrust
        F00_core = self.qoff[0, 0]
        F19_core = self.qoff[0, 19]
        F90 = self.qoff[0, 90]
        F00_fan = self.qoff_e[0, 0]
        F19_fan = self.qoff_e[0, 19]
        ...
        self.COT = self.qoff[4, 30]
        # Turbine Inlet Temperature
        self.TIT = self.qoff[4, 40]

        # Specific Fuel Consumption
        self.sfc_off = WF30 * self.g * 3600 / (F00_core + F00_fan + F19_core + F19_fan + F90)
        # Specific Thrust
        self.isp_off = ((F00_core + F00_fan + F19_core + F19_fan + F90) / self.g) / (W00_core + W00_fan)

        # BPRe, FPRe (Distributed fan)
        BPRe = W00_fan / W00_core
        FPRe = self.qoff_e[8, 10]
        ...
        OPR = 0.0
        if self.propulsion_type in ['turbojet', 'turboshaft', 'TeDP']:
            OPR = self.qoff[8, 20] * self.qoff[8, 25]
        elif self.propulsion_type in ['turbofan', 'PartialElectric']:
            OPR = self.qoff[8, 10] * self.qoff[8, 20] * self.qoff[8, 25]

        # Thrust
        self.thrust_off = self.isp_off * (W00_core + W00_fan) * self.g * self.fscl

        # Airflow rate
        self.airflow_off = self.isp_off * (W00_core + W00_fan)

        print('=' * 10 + 'Design Off Calculation Results' + '=' * 10)
        print('SFC:', self.sfc_off, 'TIT_off:', self.TIT, 'COT_off:', self.COT)
        print('BPRe_off:', BPRe, 'FPRe:', FPRe)
        print('BPR off:', BPR, 'FPR off:', FPR, 'OPR off:', OPR)
        print('Specific Thrust off:', self.isp_off)
        print('Thrust off:', self.thrust_off, 'Airflow rate:', self.airflow_off)
        print('Generator Heat Power:', self.GenePower)
        print('=' * 50)
        print('')
...
def test():
    ...
    dv_list = [30.0, 1380, 5.0, 1.5, 0.4, 0.99, 3]  # TeDP
...
def test_separate():
    ...
    # define design off point class
    cdop = CalcDesignOffPoint(aircraft_name, engine_name, aircraft_type, propulsion_type, thermal_design_variables,
                              off_param_args, design_point_params, data_base_args, cdp)
    # calculating off design point
    cdop.run_off(rev_args)
    # calculating important indexes
    cdop.objective_func_doff()
